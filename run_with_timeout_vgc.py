#!/usr/bin/env python3
"""
Concurrent version of run_with_timeout.py for VGC format that can run multiple non-conflicting battles simultaneously.
"""
...
def load_passwords():
    ...
    password_file = "passwords_vgc.json"
    try:
        with open(password_file, 'r') as f:
            return json.load(f)
    except FileNotFoundError:
        print(f"Error: {password_file} not found. Please create it from passwords_vgc.json.example")
        print("Copy passwords_vgc.json.example to passwords_vgc.json and fill in your actual passwords.")
        sys.exit(1)
...
# Define the agents and their configurations for VGC
AGENTS = [
...
def initialize_agent_passwords():
    ...
    for agent in AGENTS:
        ...
        if username not in PASSWORDS:
            print(f"Error: Password not found for username '{username}' in passwords_vgc.json")
            print(f"Available usernames: {list(PASSWORDS.keys())}")
            sys.exit(1)
        agent["password"] = PASSWORDS[username]
    print(f"Successfully loaded passwords for {len(AGENTS)} VGC agents")
...
def get_log_filename(agent1, agent2):
    ...
    return f"battle_log/vs_battles_vgc/{agent1['username']}_vs_{agent2['username']}_{timestamp}.txt"

def get_results_filename():
    """Get the results JSON filename."""
    return "battle_results_ladder_concurrent_vgc.json"
...
def run_ladder_session(agent, session_id):
    """Run a single agent on the VGC ladder."""
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    log_file = f"battle_log/ladder_vgc/{agent['username']}_{timestamp}.txt"
    ...
    # Build command for showdown_ladder.py (single agent vs ladder) - VGC format
    cmd = [
        sys.executable,
    ...
    print(f"\n[{datetime.now().strftime('%H:%M:%S')}] Starting VGC ladder session #{session_id}: {agent['username']}")
    print(f"  Games: {GAMES_PER_MATCHUP} | Timeout: {TIMEOUT_SECONDS}s | LLM Timeout: {LLM_TIMEOUT_SECONDS}s | Seed: {unique_seed}")
    print(f"  Log: {log_file}")
    print(f"  🎯 VGC LADDER BATTLES: Agent vs random opponents on Pokemon Showdown (VGC 2024 Reg H)")
    
    try:
        with open(log_file, "w") as f:
            f.write(f"===== VGC Ladder Session #{session_id}: {agent['username']} =====\n")
        ...
        print(f"[{datetime.now().strftime('%H:%M:%S')}] Completed VGC ladder session #{session_id}: {agent['username']}")
        ...
        print(f"[{datetime.now().strftime('%H:%M:%S')}] Timeout in VGC ladder session #{session_id}: {agent['username']}")
        return {
            "status": "timeout",
            "log_file": log_file,
            "timestamp": datetime.now().isoformat(),
        ...
        print(f"[{datetime.now().strftime('%H:%M:%S')}] Error in VGC ladder session #{session_id}: {e}")
        return {
            "status": "error",
            "error": str(e),
            "log_file": log_file,
...
def forfeit_existing_battles():
    ...
    print(f"\n{'='*60}")
    print("FORFEITING EXISTING VGC BATTLES")
    print(f"{'='*60}")
    ...
    for agent in AGENTS:
        print(f"Checking VGC agent: {agent['name']} ({agent['username']})")
...
def run_concurrent_ladder(skip_forfeit=False, games_per_agent_target=1000, continuous_mode=False):
    ...
    print(f"\n{'='*60}")
    print(f"CONCURRENT VGC LADDER TOURNAMENT")
    ...
    for agent in AGENTS:
        ...
        with battle_lock:
            ...
            print(f"[{datetime.now().strftime('%H:%M:%S')}] Queued VGC ladder session #{session_counter}: {username} "
    ...
    print(f"\n{'='*60}")
    print(f"ALL VGC LADDER SESSIONS COMPLETED")
...
def force_clear_battle_state():
    ...
    with battle_lock:
        if agents_in_battle:
            ...
            print(f"[{datetime.now().strftime('%H:%M:%S')}] All VGC agents forcibly freed for clean restart")
...
def run_continuous(skip_forfeit=False, games_per_agent_target=1000):
    """Run VGC ladder sessions continuously in a loop."""
    try:
        # Check battle state before starting continuous mode
        with battle_lock:
            if agents_in_battle:
                print(f"[{datetime.now().strftime('%H:%M:%S')}] WARNING: Detected VGC agents stuck in battle state: {agents_in_battle}")
                print("Force clearing battle state to allow continuous running...")
                agents_in_battle.clear()
                print(f"[{datetime.now().strftime('%H:%M:%S')}] All VGC agents forcibly freed for clean restart")
            else:
                print(f"[{datetime.now().strftime('%H:%M:%S')}] VGC battle state is clean - starting continuous mode")
        ...
        print("\nStopping VGC ladder tournament...")
    except Exception as e:
        print(f"\nError in VGC ladder tournament: {e}")
        print("Stopping continuous mode.")

if __name__ == "__main__":
    import argparse
    
    parser = argparse.ArgumentParser(description="Run concurrent Pokemon VGC ladder tournament")
    ...
    try:
        if args.continuous:
            print("Running VGC in continuous mode. Press Ctrl+C to stop.")
        ...
        print("\nExiting run_with_timeout_vgc.py")